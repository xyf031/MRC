...
MRC::MRC()
{
	...
	// These sentences below are useless:
	m_header.mode = 2;
	m_header.cellb[0] = 90;
	m_header.cellb[1] = 90;
	m_header.cellb[2] = 90;
...
int MRC::open(const char *filename, const char *mode)
{
	...
	if (fread(&m_header, 1, 1024, m_fp) < 1024) {
		printf("********** ERROR **********\nMRC file broken: mrc_header is shorter than 1024 bytes.\n");
		return -2;
	}
...
{
	// The "nline" begins from 0 meaning the line number. Usually nimage = 0.
	size_t ImSize = getImSize();
	size_t LineLength = getNx() * getWordLength();  // Here! Not "getNy"! 
...
int MRC::readPixel(void *buf, int nimage, int nline, int npixel)
{
	size_t ImSize = getImSize();
	size_t LineLength = getNx() * getWordLength();  // Here! Not "getNy"! 
	if (npixel >= (int)LineLength) return 0;
